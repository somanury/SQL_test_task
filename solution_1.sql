...
-- next two cte-s are optional: allow to clean path from page refreshing
cte_previous_page
as
(select user_id, session_id, page, happened_at,
	...
	case 
	when previous_page is not null then previous_page + ', ' + page
	else page
	end as minipath
from cte_session_id),
...
cte_cleaned
as
(select session_id, user_id, happened_at, previous_page, minipath
...
from 
	(select user_id, 
		listagg(minipath, ', ') within group (order by happened_at asc) over (partition by user_id, session_id) as path, 
